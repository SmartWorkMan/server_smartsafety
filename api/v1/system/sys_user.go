...
func (b *BaseApi) GetUserList(c *gin.Context) {
	var pageInfo systemReq.UserSearch
	...
	if strings.HasPrefix(selfUserInfo.Username, commval.MaintainUserPrefix) ||
		strings.HasPrefix(selfUserInfo.Username, commval.FactoryUserPrefix){
		global.GVA_LOG.Error("工厂管理员或者维保管理员不允许获取用户列表!")
		response.FailWithMessage("工厂管理员或者维保管理员不允许获取用户列表!", c)
	}

	pageInfo.Username = selfUserInfo.Username
