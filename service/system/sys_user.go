...
func (userService *UserService) GetUserInfoList(info systemReq.UserSearch) (err error, list interface{}, total int64) {
	...
	if info.Username == commval.SuperUser {
		err = db.Where("username != ? AND username != ? AND username != ?", commval.SuperUser, commval.RootUser, commval.AdminUser).Count(&total).Error
		if err != nil {
			return
		}
		err = db.Limit(limit).Offset(offset).Preload("Authorities").Preload("Authority").Find(&userList, "username != ? AND username != ? AND username != ?", commval.SuperUser, commval.RootUser, commval.AdminUser).Error
...
func (userService *UserService) DeleteUserByFactoryName(factoryName string) (err error) {
	var userList []system.SysUser
	err = global.GVA_DB.Where("`factory_name` = ?", factoryName).Find(&userList).Error
	if err != nil {
		return err
	}

	for _, user := range userList {
		err = userService.DeleteUser(int(user.ID))
		if err != nil {
			return err
		}
	}
